...
To make the QC rule modular for both raw and trimmed reads, I need to figure out some sort of wildcard for file paths and naming, in addition to the standard wildcard for sample.

Hmmm and we can add even more modularity by putting function definitions that might be called by multiple rules into a `common.smk` file...for example, within that common file I could have:
